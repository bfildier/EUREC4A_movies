...
def loadSondes(dtime):
    ...
    path_allsondes = os.path.join(sondedir,"all_sondes_with_time_and_cloud_flag.nc")
...
def loadMeteor(dtime):
    
    str_date = dtime.strftime("%b%d")
    path_meteor = os.path.join(meteordir,str_date+"_min.dat")
    
    df = pd.read_csv(path_meteor, 
                 engine='python',
                 skiprows=3, 
                 usecols=[8,9,10,11,13,16,17],
                 delim_whitespace=True,
                 names=['ta','ts','wdir','rh','wspd','lat','lon']
                )
    time = (pd.DataFrame(columns=['time'],
                  index=pd.date_range('2016-09-02T00:00:00Z', '2016-09-02T23:59:00Z',
                                      freq='1T'))
           .between_time('00:00','23:59')
           .index.strftime('%H:%M').tolist())

    df["ta"] = df["ta"].where(df["ta"] > 0)
...
def getMeteorObj(dtime, meteor):
    
    x, y = np.array([meteor.lon[:], meteor.lat[:]])
    line = mlines.Line2D(x, y, lw=2., alpha=1, color="r", marker="o",ms=10, markevery=[0],mfc="b",mec="b")
...
def updateMeteorObj(obj, meteor, dtime):
    
    dtime_str = dtime.strftime('%H:%M')

    matching_time = np.where(meteor.index.values[:]==dtime_str)[0]
...
def makeMovie(verbose=False):
    ...
    meteor_obj = getMeteorObj(start, meteor)
    ax.add_line(meteor_obj)
    ...
    def updateImage(i):
        ...
        updateMeteorObj(meteor_obj, meteor, dtime)
